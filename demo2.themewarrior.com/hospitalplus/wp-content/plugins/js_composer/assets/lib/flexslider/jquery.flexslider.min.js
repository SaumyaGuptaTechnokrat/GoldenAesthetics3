...
! function($) {
    ...
    $.flexslider = function(el, options) {
        var slider = $(el);
        void 0 === options.rtl && "rtl" == $("html").attr("dir") && (options.rtl = !0), slider.vars = $.extend({}, $.flexslider.defaults, options);
        var watchedEventClearTimer, namespace = slider.vars.namespace,
        ...
        $.data(el, "flexslider", slider), methods = {
            init: function() {
                ...
                }), slider.container.empty().append(slider.slides)), slider.doMath(), slider.setup("init"), slider.vars.controlNav && methods.controlNav.setup(), slider.vars.directionNav && methods.directionNav.setup(), slider.vars.keyboard && (1 === $(slider.containerSelector).length || slider.vars.multipleKeyboard) && $(document).bind("keyup", function(target) {
                    target = target.keyCode;
                    slider.animating || 39 !== target && 37 !== target || (target = slider.vars.rtl ? 37 === target ? slider.getTarget("next") : 39 === target && slider.getTarget("prev") : 39 === target ? slider.getTarget("next") : 37 === target && slider.getTarget("prev"), slider.flexAnimate(target, slider.vars.pauseOnAction))
                }), slider.vars.mousewheel && slider.bind("mousewheel", function(event, target, deltaX, deltaY) {
                    event.preventDefault();
                    target = target < 0 ? slider.getTarget("next") : slider.getTarget("prev");
            ...
            asNav: {
                setup: function() {
                    slider.asNav = !0, slider.animatingTo = Math.floor(slider.currentSlide / slider.move), slider.currentItem = slider.currentSlide, slider.slides.removeClass(namespace + "active-slide").eq(slider.currentItem).addClass(namespace + "active-slide"), msGesture ? (el._slider = slider).slides.each(function() {
                        ...
                        }, !1), this.addEventListener("MSGestureTap", function(target) {
                    ...
                    }) : slider.slides.on(eventType, function(posFromX) {
                        posFromX.preventDefault();
                        var $slide = $(this),
                            target = $slide.index(),
                            posFromX = slider.vars.rtl ? -1 * ($slide.offset().right - $(slider).scrollLeft()) : $slide.offset().left - $(slider).scrollLeft();
                        posFromX <= 0 && $slide.hasClass(namespace + "active-slide") ? slider.flexAnimate(slider.getTarget("prev"), !0) : $(slider.vars.asNavFor).data("flexslider").animating || $slide.hasClass(namespace + "active-slide") || (slider.direction = slider.currentItem < target ? "next" : "prev", slider.flexAnimate(target, slider.vars.pauseOnAction, !1, !0, !0))
                    })
                }
            },
            controlNav: {
                ...
                setupPaging: function() {
                    ...
                    if (slider.controlNavScaffold = $('<ol class="' + namespace + "control-nav " + namespace + type + '"></ol>'), 1 < slider.pagingCount)
                        for (var captn, i = 0; i < slider.pagingCount; i++) {
                            void 0 === (captn = slider.slides.eq(i)).attr("data-thumb-alt") && captn.attr("data-thumb-alt", ""), item = $("<a></a>").attr("href", "#").text(j), "thumbnails" === slider.vars.controlNav && (item = $("<img/>").attr("src", captn.attr("data-thumb"))), "" !== captn.attr("data-thumb-alt") && item.attr("alt", captn.attr("data-thumb-alt")), "thumbnails" !== slider.vars.controlNav || !0 !== slider.vars.thumbCaptions || "" !== (captn = captn.attr("data-thumbcaption")) && void 0 !== captn && (liElement = $("<span></span>").addClass(namespace + "caption").text(captn), item.append(liElement));
                            var liElement = $("<li>");
                            item.appendTo(liElement), liElement.append("</li>"), slider.controlNavScaffold.append(liElement), j++
                        }(slider.controlsContainer ? $(slider.controlsContainer) : slider).append(slider.controlNavScaffold), methods.controlNav.set(), methods.controlNav.active(), slider.controlNavScaffold.delegate("a, img", eventType, function(event) {
                            var $this, target;
            ...
            touch: function() {
                ...
                msGesture ? (el.style.msTouchAction = "none", el._gesture = new MSGesture, (el._gesture.target = el).addEventListener("MSPointerDown", function(e) {
                    ...
                    if (accDx += vertical ? transY : transX, dx = (slider.vars.rtl ? -1 : 1) * accDx, scrolling = vertical ? Math.abs(accDx) < Math.abs(-transX) : Math.abs(accDx) < Math.abs(-transY), e.detail === e.MSGESTURE_FLAG_INERTIA) return void setImmediate(function() {
                        el._gesture.stop()
                    });
                ...
                }, !1), el.addEventListener("MSGestureEnd", function(target) {
                    target.stopPropagation();
                    var slider = target.target._slider;
                    if (!slider) return; {
                        var updateDx;
                        slider.animatingTo !== slider.currentSlide || scrolling || null === dx || (target = 0 < (updateDx = reverse ? -dx : dx) ? slider.getTarget("next") : slider.getTarget("prev"), slider.canAdvance(target) && (Number(new Date) - startT < 550 && 50 < Math.abs(updateDx) || Math.abs(updateDx) > cwidth / 2) ? slider.flexAnimate(target, slider.vars.pauseOnAction) : fade || slider.flexAnimate(slider.currentSlide, slider.vars.pauseOnAction, !0))
                    }
                    offset = dx = startY = startX = null, accDx = 0
            ...
            pauseInvisible: {
                visProp: null,
                init: function() {
                    var evtname = methods.pauseInvisible.getHiddenProp();
                    evtname && (evtname = evtname.replace(/[H|h]idden/, "") + "visibilitychange", document.addEventListener(evtname, function() {
        ...
        }, slider.flexAnimate = function(slideString, pause, override, master, calcNext) {
            if (slider.vars.animationLoop || slideString === slider.currentSlide || (slider.direction = slideString > slider.currentSlide ? "next" : "prev"), asNav && 1 === slider.pagingCount && (slider.direction = slider.currentItem < slideString ? "next" : "prev"), !slider.animating && (slider.canAdvance(slideString, calcNext) || override) && slider.is(":visible")) {
                if (asNav && master) {
                    master = $(slider.vars.asNavFor).data("flexslider");
                    if (slider.atEnd = 0 === slideString || slideString === slider.count - 1, master.flexAnimate(slideString, !0, !1, !0, calcNext), slider.direction = slider.currentItem < slideString ? "next" : "prev", master.direction = slider.direction, Math.ceil((slideString + 1) / slider.visible) - 1 === slider.currentSlide || 0 === slideString) return slider.currentItem = slideString, slider.slides.removeClass(namespace + "active-slide").eq(slideString).addClass(namespace + "active-slide"), !1;
                    slider.currentItem = slideString, slider.slides.removeClass(namespace + "active-slide").eq(slideString).addClass(namespace + "active-slide"), slideString = Math.floor(slideString / slider.visible)
                }
                var dimension;
                slider.animating = !0, slider.animatingTo = slideString, pause && slider.pause(), slider.vars.before(slider), slider.syncExists && !calcNext && methods.sync("animate"), slider.vars.controlNav && methods.controlNav.active(), carousel || slider.slides.removeClass(namespace + "active-slide").eq(slideString).addClass(namespace + "active-slide"), slider.atEnd = 0 === slideString || slideString === slider.last, slider.vars.directionNav && methods.directionNav.update(), slideString === slider.last && (slider.vars.end(slider), slider.vars.animationLoop || slider.pause()), fade ? touch ? (slider.slides.eq(slider.currentSlide).css({
                    opacity: 0,
                    zIndex: 1
                }), slider.slides.eq(slideString).css({
                    opacity: 1,
                    zIndex: 2
            ...
            var posCheck, target = (posCheck = pos || (slider.itemW + slider.vars.itemMargin) * slider.move * slider.animatingTo, function() {
        ...
        if ("object" == typeof(options = void 0 === options ? {} : options)) return this.each(function() {
            var $this = $(this),
                $slides = options.selector || ".slides > li",
                $slides = $this.find($slides);
            1 === $slides.length && !1 === options.allowOneSlide || 0 === $slides.length ? ($slides.fadeIn(400), options.start && options.start($this)) : void 0 === $this.data("flexslider") && new $.flexslider(this, options)
        });
